...
### Added
- **Ops Enhancements:** Added critical helpers for the Field Operations layer:
    - **Pathfinding:** Implemented A* search (`physics.ops.pathing/a-star`) for grid-based obstacle avoidance.
    - **Frame Normalization:** Added `physics.ops.frames/ensure-frame` to automatically convert states from `:wgs84` to `:ecef` using the upgraded `physics.frames` inverse transforms.
    - **Sensors:** Added `physics.models.sensors` with standard Radar (SNR-based) and GPS (Drift-based) models for synthetic data generation.

### Fixed
- **Frames:** Implemented iterative `ecef->geodetic` and corrected `ecef->enu` matrix logic to ensure high-precision round-trip conversions (1cm tolerance).
